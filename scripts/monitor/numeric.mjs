...
For facts: retain units, cash vs gift-card method, state scope, daily vs per-transaction caps, timing stage and business vs calendar days. If unspecified say unspecified, not general. State names must be copied from evidence, not inferred from country.
...
Do not extract article dates as policy values or game counts as offers. Limit to 12 offers, 20 facts and 8 statements per operator.`;
...
for (const operator of operators) {
  ...
  const numbers = records.facts.reduce((sum, item) => sum + 1 + Number(item.upperValue !== null), 0) + records.offers.reduce((sum, item) => sum +
    ["priceUsd", "immediateSc", "totalSc", "goldCoins", "advertisedExtraPercent", "durationDays", "intervalHours"].filter(key => item[key] !== null).length, 0);
