...
try {
  for (const path of ["/", "/updates/"]) {
    ...
    for (const width of [320, 390, 768, 1024, 1440]) {
      ...
      assert.deepEqual(await page.locator(".benchmark :is(td, th, p, strong)").evaluateAll(elements =>
    ...
    assert.ok(sitemap("url").filter((_, element) => sitemap(element).find("loc").text() === `https://socialcasinoindex.com${path}`).find("lastmod").text() >= "2026-09-15");
  }
  const exports = execFileSync("git", ["ls-files", "docs/assets/data", "docs/updates"], { encoding: "utf8" })
    .trim().split("\n").filter(path => /\.(json|csv)$/.test(path));
  for (const path of exports) assert.deepEqual(await readFile(path), execFileSync("git", ["show", `161edf0:${path}`]), `Export changed: ${path}`);
