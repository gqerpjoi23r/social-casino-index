...
export const NUMERIC_VERSION = "2.5.0";
...
export const EXTRACTION_SCHEMA = object({
  offers: { type: "array", items: object({
    ...
    advertisedDiscountPercent: nullableNumber,
    extraPercentComparison: { enum: ["exact", "at_least", "greater_than", "up_to", null] },
  ...
  facts: { type: "array", items: object({
    ...
    stage: { enum: ["processing", "approval", "transfer", "end_to_end", "unspecified", "not_applicable"] },
    states: strings,
    basis: { type: "string" },
    conditions: strings,
  }) },
